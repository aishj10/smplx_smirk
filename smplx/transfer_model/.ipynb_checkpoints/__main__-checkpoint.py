...
def main() -> None:
    ...
    for ii, batch in enumerate(tqdm(dataloader)):
        ...
        var_dict, param_dict = run_fitting(
        ...
        for ii, path in enumerate(paths):
            ...
            output_path = osp.join(
                output_folder, f'{osp.splitext(fname)[0]}.npy')
            np.save(output_path,var_dict)

            output_path = osp.join(
                output_folder, f'{osp.splitext(fname)[0]}_para.npy')
            np.save(output_path,param_dict)
