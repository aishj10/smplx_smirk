...
def get_smplx_model():
    ...
    num_betas = 10 #len(reference_dict['smpls']['betas'][0])
    num_expression_coeffs = 50 #len(smirk_dict['expression_params'][0])
...
def get_smplx_smirk(model, results_dict_for_rendering, smirk_file):
    
    smirk_dict = np.load(str(smirk_file), allow_pickle=True).item()
    
    betas = torch.Tensor(results_dict_for_rendering['smpls']['betas'])
    ...
    smplx_out = {"verts" : [vertices], "joints" : [joints]}
...
if __name__ == "__main__":
    ...
    print("HMR2_DEFAULT_CKPT",HMR2_DEFAULT_CKPT)
    ...
    for img_path in tqdm(reference_img_paths, desc="Processing Reference Images:"):
        ...
        for batch in dataloader:
            ...
            smirk_file = str(os.path.join(args.reference_imgs_folder, "smirk", "smirk_param", f"{img_fn}.npy"))
            #print("==> smirk_file", smirk_file)
            result_dict_smplx_smirk = get_smplx_smirk(smplx_model, results_dict_for_rendering, smirk_file)

            #ipdb.set_trace()

            assert (len(result_dict_smplx_smirk['verts'][0].shape) == len(results_dict_for_rendering['verts'][0].shape)) and (
                    result_dict_smplx_smirk['verts'][0].shape[-1] == results_dict_for_rendering['verts'][0].shape[-1])

            results_dict_for_rendering["verts"] = result_dict_smplx_smirk['verts']
    ...
    for video_path in tqdm(driving_videos_paths, desc="Processing Driving Videos:"):
        ...
        for img_path in tqdm(driving_img_paths):
            ...
            for batch in dataloader:
                ...
                smirk_file = str(os.path.join(video_path, "smirk", "smirk_param", f"{img_fn}.npy"))
                #print("==> smirk_file", smirk_file)
                result_dict_smplx_smirk = get_smplx_smirk(smplx_model, results_dict_for_rendering, smirk_file)
