...
def get_obj(vertices, faces, output_path):
    vertex_colors = np.ones([vertices.shape[0], 4]) * [0.3, 0.3, 0.3, 0.8]
    # process=False to avoid creating a new mesh
    tri_mesh = trimesh.Trimesh(
        vertices, faces, vertex_colors=vertex_colors, process=False
    )
    # output_folder = "/data/aujadhav"
    # output_path = "/data/aujadhav/{0:04d}.obj".format(0)
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(args.output_folder, "obj_results"), exist_ok=True)
    ...
    if not have_smpl_results:
        ...
        if os.path.exists(group_smpl_path):
            ...
            for smpl_outs, cam_t, file_path in tqdm(
                ...
                #ipdb.set_trace()
                obj_path =  os.path.join( args.output_folder, "obj_results", f"{img_fn}.obj")
                get_obj(result_dict["verts"][0], model.smpl.faces, obj_path)
