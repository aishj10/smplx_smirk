...
W_FACE_AND_COLOR_FILE = this_script_path / "blend" / "smplx_mesh_info.npy"

print("====>W_FACE_AND_COLOR_FILE", W_FACE_AND_COLOR_FILE)
    ...
    def __init__(self, smpl_folder, smpl_suffixes=["npy", "npz"]):
        ...
        self.smpl_paths = sorted(
            [
                ...
                for path in (folder / "smplx_smirk_results").glob("*." + i)
            ]
        )
...
def rendering_pipeline(dataset, ref_img_path):
    ...
    verts_color = result_dict["verts_color"].astype(np.float32)

    #print("===>verts_color", verts_color.shape, result_dict["verts_color"].shape)

    result_dict_list = []

    #print("dataset.smpl_paths",dataset.smpl_paths)
    ...
    for smpl_path, output_path, result_dict in tqdm(
        ...
        transl = result_dict["transl"][0]
        verts = result_dict["verts"][0][0] + cam_t +  transl
        ...
        #print("====> verts", result_dict["verts"][0].shape)
        #print("==== faces", faces.shape)
        # make mesh
        #new_mesh = bpy.data.meshes.new("smplx_mesh")
        new_mesh = bpy.data.meshes.new("SMPL-X")
        #ipdb.set_trace()
        #new_mesh.from_pydata(verts[0], edges=[], faces=faces)
        new_mesh.from_pydata(verts, edges=[], faces=faces)
        # make object from mesh
        #new_object = bpy.data.objects.new("new_object", new_mesh)
        new_object = bpy.data.objects.new("SMPL-X_Object", new_mesh)
        # make collection
        ...
        for poly in new_mesh.polygons:
            ...
            for vert_i_poly, vert_i_mesh in enumerate(poly.vertices):
                ...
                #rgb.append(1)
                #print("===vert_i_loop, rgb",vert_i_loop, rgb, verts_color[vert_i_mesh])
